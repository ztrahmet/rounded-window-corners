...
export class WindowTracker {
    ...
    enable() {
        // enable() is reached from a promise, so a disable/enable cycle can race
        // a stale one into calling it twice. Connecting window-created twice
        // would leak a handler for the rest of the session.
        if (this._enabled || !this._alive)
            return;
        this._enabled = true;

        for (const actor of global.get_window_actors())
            this._trackSafely(actor);
    ...
    /** Nothing awaits _track, so its rejections have to be caught here. */
    _trackSafely(actor) {
        this._track(actor).catch(error =>
            logError(error, 'rounded-window-corners: could not track a window'));
    }
    ...
    _sync(actor) {
        ...
        if (!state.effect) {
            // Record the effect only once it is really attached, so a failure
            // here cannot leave the state claiming an effect that is not there.
            try {
                ...
                logError(error, 'rounded-window-corners: could not attach to a window');
                return;
            }
        }
     ...
     *
     * For X11 clients that is the surface rather than the window actor: mutter
     * paints the server-side frame shadow outside it, and leaving that shadow
     * alone is what we want, since such windows have no buffer margin in which
    ...
    _effectTarget(actor, state) {
        ...
        // appears; come back when it is. Watching the child list rather than
        // the first child matters, because a foreign actor may already hold
        // that position when the surface arrives.
...
/**
 * The child holding the window's own pixels.
 *
 * Not simply the first child. Other extensions parent actors to a window actor
 * too, and Blur My Shell inserts its blur at index 0, which would otherwise
 * take the effect and leave the window itself square. Asking for the surface by
 * type says what we actually mean; the class is not in the typelib, so it is
 * matched by name.
 */
function findSurface(actor) {
    ...
    // Mutter already wraps Wayland surfaces in a container actor, so look
    // deeper rather than lose the effect if XWayland ever gains one too.
    for (const child of children) {
