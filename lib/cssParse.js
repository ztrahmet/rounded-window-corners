...
function parseDeclarations(body) {
    ...
    for (const part of splitTop(body, ';')) {
        ...
        // `!important` has already done its job by the time we look at a
        // value, and leaving it in makes the value look unreadable below.
...
export function parseLength(value) {
    if (!value)
        return null;
    const t = value.trim();
    // The fractional part has to start with its dot rather than being two
    // optional pieces. Written the other way the pattern retries every split of
    // a long digit run before giving up, which a stylesheet from a downloaded
    // theme could turn into seconds of a stalled compositor.
 ...
 * transition.
 *
 * A layer naming a colour this reader does not cover makes the whole value
 * null, so the caller falls back to a source it can read.
 *
...
export function parseBoxShadow(value) {
    ...
    for (const part of splitTop(value, ',')) {
        ...
        // Guessing black for a colour we could not read paints an opaque ring
        // around every window. `oklch()`, `hsl()`, `currentColor` and GTK's own
        // `alpha()` all land here. Giving up on the value instead leaves the
        // next source in the cascade to supply a shadow that suits the theme.
