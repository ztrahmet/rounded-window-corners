 ...
 */
async function readStylesheet(path, depth = 0) {
    ...
    while ((match = pattern.exec(text)) !== null) {
        ...
        try {
            parts.push(await readStylesheet(child, depth + 1));
        } catch {
...
export class StyleResolver {
    ...
    async _compute() {
        ...
        if (!style) {
            ...
            const {sheets, paths} = await this._stylesheets();

            // destroy() may have run while those reads were in flight. Nothing
            // below may take effect then: _watchFiles would install monitors
            // that nobody is left to cancel, and the shell theme probe would
            // put its widget back on the stage.
            if (!this._alive)
                return this._style;
    ...
    async _stylesheets() {
        ...
        if (themeFile) {
            ...
            try {
                sheets.push(await readStylesheet(themeFile));
            } catch {
        ...
        if (GLib.file_test(userCss, GLib.FileTest.EXISTS)) {
            try {
                sheets.push(await readStylesheet(userCss));
            } catch {
