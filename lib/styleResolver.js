...
/**
 * Resolved styles, per environment, kept for as long as the shell runs rather
 * than for as long as one enable. GNOME disables extensions whenever the screen
 * locks, and reparsing a few hundred kilobytes of GTK CSS on the compositor's
 * main loop at every unlock costs more than a frame to arrive at the value we
 * already had.
 *
 * `files` is every stylesheet the last resolve read, imports included, and
 * `signature` is what those files looked like at the time, so an edit made
 * while the extension was disabled still invalidates the entries.
 */
const sessionCache = {signature: null, files: [], styles: new Map()};
 ...
 *
 * Every path reached is added to `seen`, which both keeps a file that imports
 * itself from being read again and tells the caller what to watch. A path is
 * recorded before it is read, so one that failed is still watched in case it
 * turns up later.
 *
 * Scanned by hand rather than with String.replace, which cannot await.
 */
async function readStylesheet(path, seen, depth = 0) {
    ...
    while ((match = pattern.exec(text)) !== null) {
        ...
        try {
            ...
            // A missing import contributes nothing.
        }
    }
...
export class StyleResolver {
    ...
    constructor(extensionPath, onChanged) {
        ...
        // Running the libadwaita probe is only worth it when we are about to
        // parse and have nothing to parse from, so _stylesheets() starts it.
        this._probeStarted = false;

        // A system schema from gsettings-desktop-schemas, read for the GTK
        // theme name. This extension defines no schema of its own, so it
        // correctly ships no schemas/ directory and never calls getSettings().
        this._interfaceSettings = new Gio.Settings({
            schema_id: 'org.gnome.desktop.interface',
        });

        // Reading the stylesheets is asynchronous, so start from the built-in
        // values and let the first refresh replace them. They are the same in
        // the common case, so nothing visibly changes.
    ...
    /**
     * Resolve and publish the style. Reads are asynchronous, so a refresh that
     * arrives while one is in flight is folded into a second pass rather than
     * racing it.
     */
    async _refresh() {
        if (this._refreshing) {
        ...
        try {
            ...
            logError(error, 'rounded-window-corners: could not resolve the style');
        } finally {
            this._refreshing = false;
        }
    }
    ...
    async _compute() {
        ...
        // The files a previous resolve read, plus the ones in effect now. The
        // first set covers imports, the second catches a theme switch that
        // happened while nothing of ours was listening.
        const sources = this._sources();
        const watched = [...new Set([...sources.paths, ...sessionCache.files])];

        // Watched even when the answer is already cached, since this is also
        // what makes editing a stylesheet take effect without a restart.
        ...
        if (!style) {
            ...
            // destroy() may have run while those reads were in flight, and the
            // shell theme probe below would put its widget back on the stage.
            if (!this._alive)
                return this._style;
            ...
            // Recorded from what was actually read, so both the next
            // comparison and the monitors cover the imports as well.
    ...
    /**
     * Which files the style comes from. Nothing is read here, so the cache can
     * be checked, and the files watched, without touching their contents.
     */
    _sources() {
        const themeFile = this._findThemeFile();
        // Listed whether or not it exists: creating it later is exactly the
    ...
    /**
     * What the source files look like right now, as one string. Comparing it
     * catches an edit made while the extension was disabled, which no signal of
     * ours could have reported.
     */
    _signature(paths) {
        return paths.map(path => {
            ...
            try {
                ...
                // Missing or unreadable. Either way its absence is the state.
            }
            return `${path}=${stamp}`;
        }).join('\n');
    }
    ...
    /**
     * The GTK stylesheets in effect, lowest priority first.
     *
